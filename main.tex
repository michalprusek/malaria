...
\newcommand{\knnSpecNF}{0{,}00}     % SOUTĚŽNÍ metrika: specificita @ senzitivita >= 99 %
...
Jako hlavní \textbf{soutěžní metriku} jsme zvolili \emph{specificitu při senzitivitě
$\geq 99~\%$}. U~smrtelné nemoci je totiž nepodkročitelným požadavkem \uv{nepřehlédnout
nemocného}, proto fixujeme senzitivitu vysoko (zachytit aspoň 99~\% napadených) a~soutěžíme
v~tom, kdo přitom udrží nejvyšší specificitu, tedy nejméně falešných poplachů. Odpovídá to
bodu na ROC křivce při senzitivitě 99~\%.
...
(\cref{fig:roc}) ukazuje, že schopnost \emph{seřadit} buňky není špatná: plocha pod ní je
\knnAUC{}. Naše soutěžní metrika je ale přísnější. Požadujeme zachytit aspoň 99~\% nemocných,
a~při této podmínce klesá specificita $k$-NN na \knnSpecNF{} --- aby chytil 99~\% napadených,
musel by práh posunout tak nízko, že označí prakticky všechny buňky. Požadovanou senzitivitu
tedy $k$-NN splní, ale s~nulovou specificitou --- tedy prakticky bezcenně; právě to je důvod
naučit lepší model (váš úkol).

% --- Tabulka výsledků --------------------------------------------------
\begin{table}[t]
\centering
\caption{Výsledky na skryté testovací sadě (\Ntest{} buněk). Přesnost, senzitivita
a~specificita jsou uvedeny při výchozím prahu $0{,}5$; soutěžní metrika je specificita
při senzitivitě~$\geq 99~\%$. Hodnoty jsou z~jednoho deterministického běhu.}
\label{tab:vysledky}
...
\caption{ROC křivka $k$-NN baseline na testovací sadě. Vodorovná čára značí požadovanou
senzitivitu 99~\%; soutěžní metrika je nejvyšší specificita dosažitelná nad touto čarou.
Baseline jí dosahuje až při téměř 100~\% falešně pozitivním poměru (specificita $\approx 0$).}
\label{fig:roc}
\end{figure}
...
zejména udržet co nejvyšší specificitu při požadované senzitivitě 99~\%.

% =====================================================================
% Bibliografie je v souboru refs.bib.
% Kompilace s BibTeXem: pdflatex -> bibtex -> pdflatex -> pdflatex
% (Overleaf to udělá automaticky.)
\bibliographystyle{unsrt}   % číslováno v pořadí citování
\bibliography{refs}
