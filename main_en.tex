...
\newcommand{\knnSensNF}{0.74}   % COMPETITION metric: sensitivity @ spec >= 95 %
...
As the main \textbf{competition metric} we chose the \emph{sensitivity at a specificity of
$\geq 95~\%$} --- that is, how many of the sick the model catches while allowing at most 5~\% of
false alarms. This metric directly encodes the medical requirement of ``not missing a sick
patient while not alarming needlessly'' and corresponds to the point on the ROC curve at a 5~\%
false-positive rate.
...
would have deceived us. Only looking at the full ROC curve (\cref{fig:roc}) reveals the model's
true potential: the area under it is \knnAUC{}, and if we move the threshold so as to allow 5~\%
false alarms (95~\% specificity), the sensitivity rises to \knnSensNF{}. This is our competition
score, which you will try to beat.
...
sensitivity at a specificity of $\geq 95~\%$. Values are from a single deterministic run.}
\label{tab:results}
\begin{tabular}{l S[table-format=1.2] S[table-format=1.2] S[table-format=1.2] S[table-format=1.2] S[table-format=1.2]}
\toprule
Method & {Accuracy} & {Sensitivity} & {Specificity} & {Sens.\,@\,spec.\,95\,\%} & {AUC} \\
\midrule
...
\caption{ROC curve of the $k$-NN baseline on the test set. The marked point corresponds to the
competition metric --- the highest sensitivity attainable at a specificity of at least 95~\%
(i.e.\ at most 5~\% false alarms).}
\label{fig:roc}
\end{figure}
...
in particular by raising the sensitivity while keeping the specificity high.
